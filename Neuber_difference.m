function [ sigma, i ] = Neuber_difference( E, K_, n_, K_p, sigma_e, Modus, eps)
%Neuber_difference performs the notch root approximation according to the
...
%
%[ sigma ] = Neuber_difference( E, K_, n_, K_p, sigma_e, mode, eps)
%
%Parameter:
%   E           - Youngs Modulus 
...
f       = @(sig_x) sig_x ./ (E) + 2*( sig_x / (2*K_)) .^ (1/n_) - (sigma_e/sig_x * K_p * (((sigma_e/K_p) / (E)) + 2*((sigma_e/K_p) / (2*K_))^(1/n_)));
df      = @(sig_x) 1/E + (sig_x/(2*K_))^(1/n_ - 1)/(K_*n_) + (K_p*sigma_e*(2*(sigma_e/(2*K_*K_p))^(1/n_) + sigma_e/(E*K_p)))/sig_x^2;

[sigma, i]  = newton(f,df,sigma_e,Modus, eps);